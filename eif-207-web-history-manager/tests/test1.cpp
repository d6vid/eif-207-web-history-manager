...
int TestHistoryAddPage() {
    // Prepare history with a single page
...
int TestHistoryNavigation() {
    // Prepare history with three pages
    std::deque<WebPage> pages = {
        WebPage::create("https://page1.com", "page1.com", "Page 1"),
        WebPage::create("https://page2.com", "page2.com", "Page 2"),
        WebPage::create("https://page3.com", "page3.com", "Page 3")
    };
    History history = History::create(pages, 2); // Start at page 3

    // Move left (backward in history)
...
int TestPolicyLimit() {
    Policy policy(3, std::chrono::seconds(60));  // max 3 entries in history
...
int TestPolicyClean() {
    Policy policy(5, std::chrono::seconds(60));  // 1-minute expiry

    // Prepare history timestamps with some expired entries
    auto now = std::chrono::system_clock::now();
    std::vector<std::pair<std::string, std::chrono::system_clock::time_point>> historyTimestamps = {
        {"page1", now - std::chrono::minutes(2)},  // expired
        {"page2", now - std::chrono::minutes(1)},  // expired
        {"page3", now - std::chrono::seconds(30)}, // not expired
        {"page4", now}                             // not expired
    };
...
int main() {
    int result = 0;
    result += CreateWebPageTest(); 
    result += TestHistoryCreate();
    result += TestHistoryAddPage();
    result += TestHistoryNavigation();
    result += TestPolicyLimit();
    result += TestPolicyClean();
    result += TestPolicyGetSetMax();
    result += TestPolicyGetSetDuration();
    result += TestBookmarkCreate();
    result += TestBookmarkHasTag();
    result += TestBookmarkGetTags();
    return result;
}
